...
const getBalance = async (address) => {
  data = await zilliqa.blockchain.getBalance(address);
...
describe('Transaction Tests', () => {
  ...
  test('CreateTransaction success', async () => {
    ...
    expect(await getBalance(testAccount1)).toEqual(account1balance.sub(amount).sub(gasPrice.mul(cumulativeGas)))
